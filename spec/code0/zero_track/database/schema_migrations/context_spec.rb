...
RSpec.describe Code0::ZeroTrack::Database::SchemaMigrations::Context do
  let(:connection) do
    connection = double
    pool = double
    db_config = double
    configuration_hash = { schema_migrations_path: 'db/schema_migrations' }
    allow(connection).to receive(:pool).and_return(pool)
    allow(pool).to receive(:db_config).and_return(db_config)
    allow(db_config).to receive(:configuration_hash).and_return(configuration_hash)

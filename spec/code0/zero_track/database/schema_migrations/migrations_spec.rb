...
RSpec.describe Code0::ZeroTrack::Database::SchemaMigrations::Migrations do
  let(:connection) do
    connection = double
    pool = double
    schema_migration = double
    allow(connection).to receive(:pool).and_return(pool)
    allow(pool).to receive(:schema_migration).and_return(schema_migration)
    allow(schema_migration).to receive(:create_table)
    allow(schema_migration).to receive(:table_exists?).and_return(true)

    connection
  end
  let(:context) { Code0::ZeroTrack::Database::SchemaMigrations::Context.new(connection) }

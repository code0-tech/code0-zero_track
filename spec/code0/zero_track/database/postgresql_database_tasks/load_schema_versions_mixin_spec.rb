...
RSpec.describe Code0::ZeroTrack::Database::PostgresqlDatabaseTasks::LoadSchemaVersionsMixin do
  ...
  it 'calls SchemaMigrations load_all' do
    ...
    allow(Code0::ZeroTrack::Database::SchemaMigrations).to receive(:load_all)

    instance.structure_load

    expect(instance).to have_received(:original_structure_load).ordered
    expect(Code0::ZeroTrack::Database::SchemaMigrations).to have_received(:load_all).with(connection).ordered
  end
end
